...
function setCalculatorValues(amount, rate, type, mode) {
    ...
    const typeBtn = document.querySelector(`[onclick="setTransactionType('${type}')"]`);
    if (typeBtn) {
...
function getCalculatorResults() {
    return {
        original: parseFloat(document.getElementById('originalAmount')?.textContent?.replace('₹', '').replace(/,/g, '') || 0),
        gst: parseFloat(document.getElementById('totalGst')?.textContent?.replace('₹', '').replace(/,/g, '') || 0),
        total: parseFloat(document.getElementById('totalAmount')?.textContent?.replace('₹', '').replace(/,/g, '') || 0),
        cgst: parseFloat(document.getElementById('cgst')?.textContent?.replace('₹', '').replace(/,/g, '') || 0),
        sgst: parseFloat(document.getElementById('sgst')?.textContent?.replace('₹', '').replace(/,/g, '') || 0),
        igst: parseFloat(document.getElementById('igst')?.textContent?.replace('₹', '').replace(/,/g, '') || 0)
    };
}
...
function runCalculationTest(testCase) {
    ...
    // Set values
    setCalculatorValues(testCase.amount, testCase.rate, testCase.type, testCase.mode);
    ...
    setTimeout(() => {
        ...
        // Check results
        const tests = [
            { name: 'Original Amount', actual: results.original, expected: expected.original },
...
function testCalculateAgainButton() {
    ...
    setTimeout(() => {
        ...
        const isVisible = calculateAgainBtn && calculateAgainBtn.classList.contains('show');
        console.log(`${isVisible ? '✅' : '❌'} Calculate Again button visible: ${isVisible}`);
        ...
        if (isVisible && calculateAgainBtn) {
            ...
            setTimeout(() => {
                ...
                console.log(`${afterReset === '' ? '✅' : '❌'} Input cleared: ${afterReset === ''}`);
                console.log(`${buttonHidden ? '✅' : '❌'} Button hidden after reset: ${buttonHidden}`);
            }, 100);
        }
    }, 100);
}
...
function testHistoryFeature() {
    ...
    setTimeout(() => {
        ...
        const desktopItems = desktopHistory?.querySelectorAll('.history-item') || [];
        
        console.log(`${mobileItems.length > 0 ? '✅' : '❌'} Mobile history items: ${mobileItems.length}`);
        console.log(`${desktopItems.length > 0 ? '✅' : '❌'} Desktop history items: ${desktopItems.length}`);
        ...
        if (mobileItems.length > 0) {
            ...
            setTimeout(() => {
                ...
                console.log(`${afterClick !== initialAmount ? '✅' : '❌'} History item clickable: ${afterClick !== initialAmount}`);
            }, 100);
        }
...
function testResponsiveDesign() {
    ...
    if (viewportWidth <= 768) {
        console.log(`${navHeight <= 60 ? '✅' : '❌'} Mobile navbar compact: ${navHeight}px`);
    } else {
        console.log(`${navHeight <= 100 ? '✅' : '❌'} Desktop navbar appropriate: ${navHeight}px`);
    }
}
...
function testPerformance() {
    ...
    console.log(`${calculationTime < 100 ? '✅' : '❌'} Calculation speed: ${calculationTime.toFixed(2)}ms`);
    console.log(`${errorCount === 0 ? '✅' : '❌'} JavaScript errors: ${errorCount}`);
...
function runAllTests() {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            console.log('💡 For complete testing, also test manually on different devices');
        }, 2000);
    }, 1000);
}

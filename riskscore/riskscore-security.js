...
(function() {
    ...
    function switchTab(language) {
        ...
        const activeButton = document.querySelector(`[data-language="${language}"]`);
        if (activeButton) {
            activeButton.classList.add('active');
        }
        
        // Show corresponding panel
        const panelId = codePanels[language];
        if (panelId) {
    ...
    function initTabSwitcher() {
        ...
        document.querySelectorAll('.tab-button').forEach(button => {
            const language = button.textContent.toLowerCase().replace('.js', 'js');
    ...
    function analyzeEmailPattern(email) {
        ...
        if (email.includes('10minutemail') || email.includes('tempmail') || email.includes('guerrilla')) {
            return {
                risk_score: 97,
                confidence: 95,
                recommendation: "HIGH_RISK",
                signals_detected: ["🚨 Disposable email provider detected", "⚠️ Temporary email service"]
            };
        } else if (email.includes('microsoft') || email.includes('google') || email.includes('amazon')) {
            return {
                risk_score: 15,
                confidence: 85,
                recommendation: "LOW_RISK",
                signals_detected: ["✅ Major corporation email", "👔 Professional domain"]
            };
        } else if (email.match(/\d{4}/) || email.includes('gamer') || email.includes('cute') || email.includes('cool')) {
            return {
                risk_score: 78,
                confidence: 70,
                recommendation: "HIGH_RISK",
                signals_detected: ["⚠️ Suspicious pattern detected", "🎮 Gaming-related username"]
            };
        } else if (email.includes('dr.') || email.includes('prof') || email.match(/^[a-z]+\.[a-z]+@/)) {
            return {
                risk_score: 25,
                confidence: 75,
                recommendation: "LOW_RISK",
                signals_detected: ["👔 Professional pattern", "✅ Formal email structure"]
            };
        } else {
            return {
                risk_score: 45,
                ...
                signals_detected: ["ℹ️ Standard email provider", "🔍 Additional verification recommended"]
            };
        }
    }
...
${signals}
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        console.log('✅ RiskScore security JS loaded');
    });
